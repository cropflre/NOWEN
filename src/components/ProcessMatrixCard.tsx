          ...
          >
            ...
            <div
              ...
              style={{
                ...
                borderTop: isDark
                  ? "5px solid rgba(34, 197, 94, 0.3)"
                  : "5px solid rgba(34, 197, 94, 0.4)",
              }}
            />
          </motion.div>
        )}
...
function MissionTimer({
  ...
  return (
    <div className="flex items-center gap-1.5 font-mono text-[10px]">
      ...
      />
      <span
        className={cn(isDark ? "text-green-500/50" : "text-emerald-600/60")}
      >
        {missionTime}
      </span>
    </div>
    ...
    >
      ...
      <div className="flex-shrink-0">
        <motion.div
          ...
          animate={
            isRunning ? { scale: [1, 1.3, 1], opacity: [0.7, 1, 0.7] } : {}
          }
...
export function ProcessMatrixCard({ className }: { className?: string }) {
  ...
  const [isCollapsed, setIsCollapsed] = useState(() => {
    try {
      return localStorage.getItem(PM_COLLAPSE_KEY) === "1";
    } catch {
      return false;
    }
  });
  ...
  const toggleCollapse = useCallback(() => {
    setIsCollapsed((prev) => {
      const next = !prev;
      try {
        localStorage.setItem(PM_COLLAPSE_KEY, next ? "1" : "0");
      } catch {
        /* */
      }
      return next;
  ...
  } = useSWR<DockerContainer[]>("/api/system/docker", fetcher, {
  ...
  const handleDockerAction = useCallback(
    async (containerId: string, action: "start" | "stop" | "restart") => {
      ...
      try {
        const res = await fetch(`/api/system/docker/${containerId}/${action}`, {
          method: "POST",
        });
  ...
  const tabs: { key: TabType; icon: React.ReactNode; label: string }[] = [
    {
      key: "matrix",
      icon: <Grip className="w-3 h-3" />,
      label: t("monitor.tab_matrix", "光点"),
    },
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="ml-auto flex items-center gap-2 text-[10px] font-mono flex-shrink-0">
            <div className="flex items-center gap-1">
              ...
              <span
                className={
                  isDark ? "text-green-400" : "text-emerald-600 font-medium"
                }
              >
                {stats.running}
              </span>
            ...
            <div className="flex items-center gap-1">
              ...
              />
              <span className={isDark ? "text-red-400/70" : "text-red-500"}>
                {stats.stopped}
              </span>
            </div>
            {/* 折叠切换 */}
                      ...
                      <div className="mb-4 space-y-1.5 max-h-[200px] overflow-y-auto scrollbar-thin pr-1">
                        ...
                        >
                          ...
                          <span>
                            <span
                              style={{
                                color: getStateColor("running", isDark),
                              }}
                            >
                              {stats.running} 运行
                            </span>
                            {stats.stopped > 0 && (
                              <>
                                ...
                                <span
                                  style={{
                                    color: getStateColor("exited", isDark),
                                  }}
                                >
                                  {stats.stopped} 停止
                                </span>
