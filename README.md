...
| **AI 助手**        | AI 智能标签 · 批量 AI 分类 · 批量 AI 元数据&图标 · AI 对话助手 · 多语言输出              |
...
**⚠️ 重要**：务必将此目录映射到宿主机固定路径，否则容器重建/更新后数据会丢失！

```yaml
volumes:
  # 推荐：使用宿主机绑定挂载（更新容器不丢数据）
  - ./nowen-data:/app/server/data
  # 或者 NAS 用户指定绝对路径
...
- [x] ~~书签标签系统~~ ✅ v0.2.0 已实现（AI 智能标签 + 彩色标签药丸）
- [x] ~~AI 智能分类与元数据~~ ✅ v0.2.0 已实现（批量 AI 分类/元数据/图标）
- [x] ~~AI 对话助手~~ ✅ v0.2.0 已实现（语义搜索 + 书签卡片）
- [x] ~~WebDAV 云备份~~ ✅ v0.2.1 已实现（WebDAV 云端 + 本地备份 + 定时自动备份）
